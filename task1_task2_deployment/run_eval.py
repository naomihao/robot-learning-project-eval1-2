...
def _return_to_pose(args: argparse.Namespace, reset_pose: dict[str, float] | None) -> None:
    ...
    try:
        ...
        keys = [key for key in reset_pose if key in current_pose]
        steps = max(1, int(args.reset_duration_s * args.reset_fps))
        period_s = 1.0 / args.reset_fps
        for step in range(1, steps + 1):
            ratio = step / steps
            action = {
                key: current_pose[key] + (reset_pose[key] - current_pose[key]) * ratio
                for key in keys
            }
            robot.send_action(action)
            time.sleep(period_s)
        robot.send_action({key: reset_pose[key] for key in keys})
    finally:
        if robot.is_connected:
            robot.disconnect()
...
def _load_normalizer(
    ...
    *,
    ...
    if enable_vlm_fallback:
        raise RuntimeError("Standalone VLM/LLM prompt fallback is disabled for eval deployment.")

    print("[task2] Using deterministic prompt preprocessing only (no extra VLM model).")
...
def _run_round(args: argparse.Namespace, task: str) -> None:
...
def run_task2(args: argparse.Namespace) -> None:
    ...
    normalizer = None if args.skip_task2_normalization else _load_normalizer(
    ...
    if args.interactive:
        for i, raw_task in _interactive_prompts(args):
            print(f"\n[task2] Rollout {i}")
            try:
                task = _prepare_task2_prompt(args, normalizer, raw_task)
            except ValueError as exc:
                print(f"[task2] {exc}")
                continue
            _run_round(args, task)
    ...
    for i in range(args.n_rollouts):
        ...
        task = _prepare_task2_prompt(args, normalizer, args.task)
        _run_round(args, task)


def _prepare_task2_prompt(args: argparse.Namespace, normalizer, raw_task: str) -> str:
...
def _add_common_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--backend",         choices=("auto", "rollout", "record", "template"),
                   default=BACKEND,
                   help="Backend selector. auto uses lerobot-rollout if present, otherwise lerobot-record.")
    ...
    p.add_argument("--device",          default=DEVICE,
                   help="Device for policy/VLM. Default: env LEROBOT_DEVICE or auto.")
    ...
    p.add_argument("--skip-task2-normalization", action="store_true",
                   help="Dry-run only: skip task2 camera/VLM normalization and pass prompts through.")
    p.add_argument("--enable-vlm-fallback", action="store_true",
                   help="Disabled for eval: prompt preprocessing must stay deterministic and use no extra model.")
...
def main() -> None:
    ...
    if args.record_num_episodes is not None and args.record_num_episodes <= 0:
        ...
        sys.exit(2)
    if args.backend not in {"auto", "rollout", "record", "template"}:
        print("error: --backend must be one of auto, rollout, record, template", file=sys.stderr)
        sys.exit(2)
